...
class MultiServiceFLEnvironment:
    ...
    def get_observation(self, service_id: int) -> np.ndarray:
        """获取特定服务的观察."""
        return self.observations[service_id].to_array()
    ...
    def render(self, mode='human'):
        ...
        if mode == 'human':
            ...
            for service_id, obs in self.observations.items():
                print(f"服务 {service_id}:")
                print(f"  准确率: {obs.fl_state.accuracy:.3f}")
                print(f"  损失: {obs.fl_state.loss:.3f}")
                print(f"  能量: {obs.system_state.total_energy:.6f} J")
                print(f"  延迟: {obs.system_state.total_delay:.6f} s")
                print(f"  通信量: {obs.system_state.communication_volume} bits")
...
def test_mdp_framework():
    ...
    env = MultiServiceFLEnvironment(service_ids, constraints, max_rounds=5)
    ...
    print(f"初始观察形状: {observations[1].shape}")
    ...
    for round_num in range(3):
        ...
        actions = {}
        for service_id in service_ids:
            actions[service_id] = env.action_space.sample()
